...
def send_trajectory_handler(entry_x0, entry_y0, entry_z0,
    ...
    try:
        ...
        if not (-397 <= z0_val <= -307.38): messagebox.showerror("Lỗi", f"Z0={z0_val:.2f} nằm ngoài giới hạn robot"); return
        ...
        if not (-397 <= zf_val <= -307.38): messagebox.showerror("Lỗi", f"Zf={zf_val:.2f} nằm ngoài giới hạn robot"); return
